...
/* * * * * * * * * * * * * * * * * * * * * * * *\
 * BLELISTENER 
 * Reports straight to the Hub 
 * Handles setting up and tearing down connections
 * 
 * * * * * * * * * * * * * * * * * * * * * * * */	
class BLEConnectionListener implements BGAPIListener{
	...
	public BLEConnectionListener(BLEManager manager){
	...
	public void receive_connection_status(int conn, int flags, BDAddr address, int address_type, int conn_interval, int timeout, int latency, int bonding) {
		...
		if (flags != 0) {
			bledevice = manager.devList.getFromAddress(address.toString());
		...
		if (discovery_state != IDLE && bledevice != null) {
			...
			if (discovery_state == ATTRIBUTES) {
				if (discovery_it.hasNext()) {
					...
					manager.bgapi.send_attclient_find_information(connection, discovery_srv.getStart(), discovery_srv.getEnd());
				}
